...
function dateToTimestamp(date) {
  ...
  const millisecondsTimestamp = dateObj.getTime();
...
function formatDate(date) {
  ...
  const month = dateNew.getUTCMonth() + 1;
  ...
  const seconds = dateNew.getUTCSeconds();
...
function getWeekNumberByDate(date) {
  ...
  const dayOfWeek = dateNew.getDay() === 0 ? 7 : dateNew.getDay();
  dateNew.setDate(dateNew.getDate() + 1 - dayOfWeek);
  const firstDayOfYear = new Date(dateNew.getFullYear(), 0, 1);
  const diff = dateNew - firstDayOfYear;
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 12; i += 1) {
    if (dateNew.getDay() === 5 && dateNew > date) {
      return dateNew;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let inWorkCycle = true;

  while (currentDate <= endDate) {
    if (inWorkCycle) {
      if (workDaysCounter < countWorkDays) {
        schedule.push(formatDateSchedule(currentDate));
        workDaysCounter += 1;
      }
      if (workDaysCounter === countWorkDays) {
        inWorkCycle = false;
      ...
      offDaysCounter += 1;
      if (offDaysCounter === countOffDays) {
        inWorkCycle = true;
        offDaysCounter = 0;
      }
    }
